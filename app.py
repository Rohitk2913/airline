...
def book_ticket(ticket_id):
    ...
    try:
        ...
        ticket_status = ticket[2]  # Assuming 'statuses' is the third column
...
def add_ticket():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Redirect to the ticket.html page instead of home.html
            return redirect(url_for('ticket'))  # Adjusted redirection to the 'ticket' route
        except Exception as e:
...
def ticket():
    # Fetch tickets from the database and pass them to the template
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        cursor.execute("SELECT * FROM Tickets WHERE users = ?", (session['username'],))
...
def update_ticket(ticket_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('ticket'))  # Adjusted to redirect to 'ticket' route
        except Exception as e:
